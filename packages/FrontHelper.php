...
class FrontHelper {
	...
	/**
	 * Wrapper for wp_list_categories() that can mark the "All" item as active.
	 *
	 * @param array|string $args Arguments passed to wp_list_categories().
	 *
	 * @return string
	 */
	public static function wp_list_categories( array|string $args = '' ): string {
		$defaults = array(
			'echo' => false,
		);
		...
		// Add current-cat to the built-in "All" item when no taxonomy term is currently queried.
		if ( $out && ! ( is_category() || is_tag() || is_tax() ) ) {
			$out = preg_replace(
				'/(<li[^>]+class=["\']?)([^"\']*\bcat-item-all\b[^"\']*)(["\']?[^>]*>)/i',
				'$1$2 current-cat $3',
				$out,
				1
			);
