...
class ContentHelper {
	...
	/**
	 * Find the position of a paragraph in the content.
	 *
	 * @param string $content
	 * @param int    $p_number Which paragraph to target. Defaults to 1 (first paragraph).
	 *
	 * @return int|false The position of the paragraph, or false if not found.
	 */
	public static function find_opening_p_position( string $content, int $p_number = 1 ): int|false {
		$offset = 0;

		for ( $i = 1; $i <= $p_number; $i ++ ) {
			$p_gutenberg_pos = strpos( $content, '<!-- wp:paragraph', $offset );
			$p_classic_pos   = strpos( $content, '<p', $offset );
			$found           = false;

			if ( $p_gutenberg_pos === false && $p_classic_pos === false ) {
				// No more paragraphs found
				return false;
			}

			// Find the earliest occurrence of a paragraph opening
			if ( $p_gutenberg_pos !== false && $p_classic_pos !== false ) {
				$found = ( $p_gutenberg_pos < $p_classic_pos ) ? $p_gutenberg_pos : $p_classic_pos;
			} elseif ( $p_gutenberg_pos !== false ) {
				$found = $p_gutenberg_pos;
			} elseif ( $p_classic_pos !== false ) {
				$found = $p_classic_pos;
			}

			// If this is the paragraph we're targeting, return its position (before the opening)
			if ( $i === $p_number ) {
				return $found;
			}

			// Move offset just after the found position so next iteration looks for the following paragraph
			$offset = $found + 1;
		}
